...
const caniuseLite = require('caniuse-lite');
...
function normalizeBrowserVersion(browser, givenVersion, versions = getSortedBrowserVersions(browser), allowSmaller = false) {
    ...
    if (allowSmaller && semver.lt(givenVersionCoerced, ensureSemver(browser, closestMatch), { loose: true })) {
        return givenVersion;
    }
    return closestMatch;
}
...
const compatData = require$1('@mdn/browser-compat-data');
/**
...
function getCaniuseLiteFeatureNormalized(stats, featureName) {
    ...
    keys.forEach(browser => {
        ...
        Object.entries(browserDict).forEach(([version, support]) => {
            ...
            const normalizedVersion = versionMatch == null ? version : versionMatch[1];
            let supportKind;
            if (support === CaniuseSupportKind.AVAILABLE ||
...
function normalizeFeature(feature) {
    if (feature in CANIUSE_TO_MDN_FEATURE_MAP) {
        return CANIUSE_TO_MDN_FEATURE_MAP[feature];
    }
    return feature;
}
/**
...
function getMdnFeatureSupport(feature) {
    const match = objectPath.get(compatData, feature);
...
function generateBrowserslistFromUseragent(useragent) {
    ...
    if (semver.lt(finalVersionCoerced, closestMatchingCaniuseBrowserVersionCoerced, { loose: true }) &&
