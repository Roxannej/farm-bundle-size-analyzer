...
export type ModuleType = LiteralUnion<'ts' | 'js' | 'jsx' | 'tsx' | 'css' | 'html' | 'asset'>;
export type ResolveKind = {
    entry: string;
} | LiteralUnion<'import' | 'dynamicImport' | 'require' | 'cssAtImport' | 'cssUrl' | 'scriptSrc' | 'linkHref' | 'hmrUpdate'>;
import type { WatchOptions } from 'chokidar';
import type { JsPlugin, LiteralUnion } from '../plugin/type.js';
import { JsMinifyOptions, ScriptDecoratorsConfig, ScriptParseConfig, SwcPresetEnvOptions } from './swc-config.js';
...
type BrowserTargetsRecord = Partial<Record<'chrome' | 'opera' | 'edge' | 'firefox' | 'safari' | 'ie' | 'ios' | 'android' | 'node' | 'electron', string>> & {
    [key: string]: string;
};
export interface OutputConfig {
     ...
     * @default 'browser'
     */
    targetEnv?: 'browser' | 'node' | 'node16' | 'node-legacy' | 'node-next' | 'browser-legacy' | 'browser-es2015' | 'browser-es2017' | 'browser-esnext' | 'library' | 'library-browser' | 'library-node';
    /**
