...
import { generateFileTree, generateFileTreeHtml, stripQueryAndHash } from '../../utils/index.js';
...
export function resourcesMiddleware(compiler, serverContext) {
    return async (ctx, next) => {
        await next();
        if (ctx.method !== 'HEAD' && ctx.method !== 'GET')
            return;
        const hasHtmlPathWithPublicDir = path.resolve(serverContext.publicDir, 'index.html');
        ...
        if ((ctx.body || ctx.status !== 404) && !isSkipPublicHtml)
            return;
        ...
        const resourceResult = findResource([url, stripQueryAndHashUrl], compiler, publicPath);
        ...
        // if resource is image or font, try it in local file system to be compatible with vue
        {
            ...
            const absPath = path.join(compiler.config.config.root, resourceWithoutPublicPath);
            ...
            const absPathPublicDir = path.resolve(compiler.config.config.root, compiler.config.config.assets.publicDir, resourceWithoutPublicPath);
...
export function resources(devSeverContext) {
    ...
    if (!devSeverContext.config.writeToDisk) {
        ...
        middlewares.push(koaStatic(devSeverContext.getCompiler().config.config.output.path, {
            extensions: ['html']
        }));
    }
