...
import { FARM_TARGET_NODE_ENVS, clearScreen } from '../index.js';
import { logError } from '../server/error.js';
import { PersistentCacheBrand, bold, green } from './color.js';
export async function compilerHandler(callback, config, logger, options) {
    ...
    logger.info(`Build completed in ${bold(green(`${elapsedTime}ms`))} ${persistentCacheText} Resources emitted to ${bold(green(output.path))}.`);
}
